...
solar = solar.resample('60min').mean()
...
fail,dispatch = peak_shaving_sim(   df[['load',f'solar_{angle}',f'netload_{angle}']][:24*4],
                                    f'netload_{angle}',
                                    batt_kwh,
                                    thresholds,
                                    TOU,
...
fail,dispatch = peak_shaving_sim(  df[['load',f'solar_{angle}',f'netload_{angle}']][:96],
                                          f'netload_{angle}',
                                          batt_kwh,
                                          thresholds,
                                          TOU,
...
# %%
output = pd.read_csv(output_filename,#'Output\caltech_ev_mjw6__output__23-07-31_13-34-06.csv',
                     index_col=0)
reduc = {}
for angle in ['w90']:
