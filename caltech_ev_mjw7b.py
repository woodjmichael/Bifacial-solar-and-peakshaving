...
# %%
dir = './Data/'
output_filename = 'Output/caltech_ev_mjw7b__output__15min-doubleSolar-peak16-21__'+pd.Timestamp.now().strftime('%y-%m-%d_%H-%M-%S')+'.csv'
...
# fail,dispatch = peak_shaving_sim(  df[['load',f'solar_{angle}',f'netload_{angle}']].loc['2018-5'],
#                                           f'netload_{angle}',
#                                           batt_kwh,
#                                           thresholds,
#                                           TOU,
...
for angle in ['s20','w90','s20w90']:#'s20, w20','w90','s20w90'
    for batt_kwh in [25,50,100,150,200,400,600,1000]:
            ...
            for month in df_year.index.month.unique():
                ...
                for th0 in range(max(0,int(best_th0-rough_step)),int(best_th0+rough_step+1),3):
                    for th1 in range(max(0,int(best_th1-rough_step)),int(best_th1+rough_step+1),3):
                        for th2 in range(max(0,int(best_th2-rough_step)),int(best_th2+rough_step+1),3):                        
                            k += 1
                            fail,dispatch = peak_shaving_sim(   df_month,
                                                                f'netload_{angle}',
                                                                batt_kwh,
...
# %%
#output_filename = 'Output/caltech_ev_mjw7__output__15min-no-scaling_23-07-31_22-45-34.csv'
output = pd.read_csv(output_filename,
                     index_col=0)
reduc = {}
for angle in ['w90','s20w90']:
